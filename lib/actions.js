...
module.exports = actions = {
      ...
      applicationHelpers.startAll(function() {
        ...
        configHelpers.watcher.one(function () {
          ...
          if (cozyLight.getStatus() === 'started') {
            logger.info('Cozy Light is restarting...');
            actions.restart(program);
          }
        });

        logger.info('Cozy Light is started!');
  ...
  stop: function (callback) {
    ...
    if (cozyLight.getStatus() === 'started') {
      ...
      return applicationHelpers.stopAll(function (appErr) {
        if (appErr) {
          logger.raw(appErr);
          logger.raw(appErr.stack);
          logger.warn('An error occurred while stopping applications');
        }
        ...
        pluginHelpers.stopAll(function (pluginErr) {
          if (pluginErr) {
            ...
            logger.raw(pluginErr.stack);
            logger.warn('An error occurred while stopping plugins');
          }
            ...
            var setupCb = function(installErr){
              if (installErr) {
                ...
                if(cozyLight.getStatus() !== 'started') {
                  ...
                  actions.once('restarted', callback);
                  configHelpers.addApp(appName, manifest);
                }
              }
            };
   ...
   * @param {Object} program Commander cli object.
   * @param {Function} callback Termination.
   */
            ...
            var setupCb = function (installErr) {
              ...
              if (installErr) {
                ...
                if(cozyLight.getStatus() !== 'started') {
                  ...
                  actions.once('restarted', function (){
                    nodeHelpers.invoke(callback);
                  });
                  configHelpers.watcher.one(function () {
                    pluginHelpers.loadPlugin(pluginName, program);
